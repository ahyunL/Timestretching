# -*- coding: utf-8 -*-
"""UROP_TimeStretch(원본+all)_small.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/16Tf75iwwJhaph_BD2rSv0UPO8laSlOdZ
"""
...
import datasets, evaluate, transformers, fsspec, gcsfs, sys, os
...
# 🔒 전역 캐시 비활성화 (datasets의 map 캐시 등)
datasets.disable_caching()
print("HF Datasets caching disabled.")
...
proc_train_orig = train_dataset.map(
    prepare_dataset,
    remove_columns=[c for c in train_dataset.column_names if c not in keep],
    load_from_cache_file=False,  # ← 캐시 무시
    desc="prep_train_orig"
)
proc_val_orig   = val_dataset.map(
    prepare_dataset,
    remove_columns=[c for c in val_dataset.column_names   if c not in keep],
    load_from_cache_file=False,
    desc="prep_val_orig"
)
proc_test_orig  = test_dataset.map(
    prepare_dataset,
    remove_columns=[c for c in test_dataset.column_names  if c not in keep],
    load_from_cache_file=False,
    desc="prep_test_orig"
)
...
# ⚠️ 학습 전에 항상 새로 로드해서 이전 파인튜닝 가중치 영향 제거
...
def augment_copy(ds_raw, rate: float):
    # rate를 fn_kwargs로 전달하여 fingerprint에 반영 + 캐시 무시
    ds_aug = ds_raw.map(
        _aug_one,
        fn_kwargs={"rate": rate},
        load_from_cache_file=False,
...
# ★ 원본 + (0.7, 0.9, 1.1, 1.3) 각각 200개씩 = 800개
...
args = Seq2SeqTrainingArguments(
    output_dir="./whisper-small-mix_all4_fresh",  # 새 디렉토리
...
# 🔁 혹시 이전에 model이 학습되었을 수 있으므로, 학습 직전에 다시 로드(중복 안전)
model = WhisperForConditionalGeneration.from_pretrained(model_checkpoint).to(device)
model.config.use_cache = False
model.config.forced_decoder_ids = None
model.config.suppress_tokens = []
...
def make_processed_with_rate(ds_raw, rate: float):
    ds_aug = ds_raw.map(
        _aug_one_eval, fn_kwargs={"rate": rate},
        load_from_cache_file=False, desc=f"tempo_test_{rate}"
    )
    ds_proc = ds_aug.map(
        prepare_dataset,
        remove_columns=[c for c in ds_aug.column_names if c not in {"input_features","labels","locale"}],
        load_from_cache_file=False, desc=f"prep_test_{rate}"
    )
    ds_proc.set_format(columns=["input_features","labels","locale"])
    return ds_proc
...
df_all.to_csv("/content/results_mix_all4_fresh_test_orig_and_rates.csv", index=False)
print("Saved -> /content/results_mix_all4_fresh_test_orig_and_rates.csv")

import matplotlib.pyplot as plt

pd.options.display.float_format = '{:.6f}'.format  # 소수 6자리로 비교
